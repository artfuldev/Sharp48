            ...
            foreach (
                var tiles in
                    MonotonicityPaths.Select(
                        monotonicityPath =>
                            monotonicityPath.Select(t => game.Grid.Squares.ElementAt(t).GetSafeTileValue()))
                        .Select(tiles => tiles as uint[] ?? tiles.ToArray()))
            {
                if (IsMonotonicallyDecreasing(tiles))
                    score += tiles.First();
                else if (IsMonotonicallyDecreasing(tiles.Reverse()))
                    score += tiles.Last();
            }
            return score;
        }
